...
const betK3 = async (req, res) => {
    try {
        ...
        if (gameJoin == 1) {
            ...
            let twoSame = listJoin.split('@')[0]; // Choose 2 matching numbers
            let motDuyNhat = listJoin.split('@')[1]; // Choose a unique pair of numbers
            if (twoSame.length > 0) {
            ...
            let baDuyNhat = listJoin.split('@')[0]; // Choose 3 unique numbers
            let countBaDuyNhat = 0;
            if (baDuyNhat.length > 0) {
                countBaDuyNhat = baDuyNhat.split(',').length;
            }
            let threeSame = listJoin.split('@')[1].length; //Choose 3 of the same numbers
            total = money * xvalue * countBaDuyNhat + (threeSame * money * xvalue);
        } else if (gameJoin == 4) {
            let threeNumberUnlike = listJoin.split('@')[0]; // Choose 3 unique numbers
            let twoLienTiep = listJoin.split('@')[1]; // Choose 3 consecutive numbers
            let twoNumberUnlike = listJoin.split('@')[2]; // Choose 3 unique numbers
...
async function plusMoney(game) {
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        if (orders.typeGame == "total") {
            ...
            nhan_duoc += price * get;
            await connection.execute('UPDATE `result_k3` SET `get` = ?, `status` = 1 WHERE `id` = ? ', [receive, id]);
            const sql = 'UPDATE `users` SET `money` = `money` + ? WHERE `phone` = ? ';
            await connection.execute(sql, [receive, phone]);
        }
        nhan_duoc = 0;
