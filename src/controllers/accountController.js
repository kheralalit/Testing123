...
const verifyCode = async(req, res) => {
    ...
    if (rows.length == 0) {
        await request(`http://51.79.175.197:7800/sms/batch/v2?appkey=NFJKdK&appsecret=brwkTw&phone=91${phone}&msg=Your verification code is ${otp}&extend=${now}`,  async(error, response, body) => {
        ...
        if (user.time_otp - now <= 0) {
            request(`http://51.79.175.197:7800/sms/batch/v2?appkey=NFJKdK&appsecret=brwkTw&phone=91${phone}&msg=Your verification code is ${otp}&extend=${now}`,  async(error, response, body) => {
                let data = JSON.parse(body);
                if (data.code == '00000') {
